...
const initialState = {
  tweets: [0, 3, 98, 0, 0].map((likes, i) => ({
    id: i + 1,
    likes,
    username: `${shouldFail(i + 1) ? 'Fail' : 'Cool'}Bot${i + 1}`,
...
class App extends Component {
  ...
  onClickLike(tweetId) {
    ...
    if (component.likeRequestPending) {
      console.log('Like request already pending! Do nothing.');
      return;
    }

    console.log(`Update state: ${tweetId}`);
    const isLiked = component.state.likedTweets.includes(tweetId);
    component.setState(setTweetLiked(tweetId, !isLiked));
    component.likeRequestPending = true;
    ...
    likeTweetRequest(tweetId, true)
      ...
      .then(() => {
         component.likeRequestPending = false;
      });
  };
